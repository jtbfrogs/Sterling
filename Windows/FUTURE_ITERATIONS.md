...
- **Real pixel vision (VLM)** — a vision-language model that actually *understands* what the camera sees, not just object labels (see below)
...
| Vision | Webcam (YOLO labels) | YOLO + VLM (moondream/llava — real understanding) |
| Memory | Keyword recall + facts (JSON) | ChromaDB semantic + facts |
...
### Vision-Language Model (VLM) — Real Pixel Understanding ⭐

**The single biggest vision upgrade.** Right now vision is YOLO-only: it returns 80 fixed object
labels ("person, cup, laptop"). A VLM takes the actual camera frame + a question and answers in
plain English about *anything* — colours, text, what you're doing, facial expression, the state
of the room. The code hook is **already built** (`vision.vlm_model` in config, `_describe_scene()`
in main.py, `LLM.describe_image()`); enabling it is one `ollama pull` + one config line.

**Constrained on the M1 8 GB** — moondream fits, but running it alongside a 7B LLM + Whisper +
camera is tight, so on the Mac it's an optional, occasional-use feature. **On the Windows 3060
build it's free real estate** — plenty of VRAM to keep a VLM resident *and* run an 8B LLM.

| | YOLO (always-on) | VLM (on vision questions) |
|---|---|---|
| Output | fixed object labels + boxes | natural description of anything |
| "What am I holding?" | "cell phone" | "an iPhone with a cracked screen" |
| "Describe the scene" | "person, laptop, cup" | "you're at a messy desk, coffee on the right, looking focused" |
| Reads text / colours / detail | ❌ | ✅ |
| Speed | instant | ~1–3 s (M1) / sub-second (3060) |
| Role | tracking, presence, gestures | answering "what do you see" |

They are **complementary, not either/or** — keep YOLO for the fast always-on stuff (object
tracking, presence, gestures) and call the VLM only when the user actually asks a vision question.
Pair the VLM answer with YOLO labels for grounding (already done in `_describe_scene()`).

**Model options (smallest → most capable):**
| Model | Size | Notes |
|---|---|---|
| `moondream` | ~1.7 GB | Best pick for tight RAM — fast, great at "what is this" |
| `llava-phi3` | ~2.9 GB | Middle ground, sharper reasoning |
| `llava:7b` | ~4.7 GB | More detailed/accurate, slower — comfortable on a 3060 |
| `bakllava` | ~4.7 GB | Mistral-based LLaVA variant |

```bash
# Mac (occasional use):
ollama pull moondream
# Windows 3060 (keep it resident, more capable):
ollama pull llava:7b
```
```yaml
# config.yaml
vision:
  vlm_model: "moondream"   # or "llava:7b" on the 3060. Blank = YOLO-only.
```

**Windows VRAM budget with VLM resident:**
```
Llama 3.1 8B (Q4)        ~5.5 GB
llava:7b (VLM)           ~4.7 GB   ← swaps in on vision queries
Whisper Large-v3         ~3.0 GB
─────────────────────────────────
Ollama will hot-swap LLM/VLM as needed; 12 GB handles it. On 16 GB system RAM
the two models can stay warm and responses feel instant.
```

**Future VLM ideas once the hardware allows:**
- Continuous scene awareness (low-FPS VLM loop) → "hey, you left the stove on"
- Read documents/whiteboards held up to the camera
- Visual memory: "what was I working on yesterday?" with periodic VLM snapshots
- Describe people's expressions/mood for more natural greetings
