...
class WebcamVision:
    ...
    def get_frame_jpeg(self, quality: int = 80) -> Optional[bytes]:
        """Return the latest frame encoded as JPEG bytes (for VLM input)."""
        ...
        if self._use_face_recog:
            ...
            for i, (loc, enc) in enumerate(zip(locations, encodings)):
                ...
                if self._known_encodings:
                    matches   = _fr.compare_faces(self._known_encodings, enc, tolerance=self._face_tolerance)
                ...
                md = self._face_merge_dist
                for idx, det in enumerate(detections):
                    if det["label"] == "person":
                        dcy = (det["box"][1] + det["box"][3]) // 2
                        if abs(det["cx"] - face_cx) < md and abs(dcy - face_cy) < md:
                            det["label"] = name
                            # Expression hint
                            if i < len(landmarks):
    ...
    def _apply_face_recognition(
        ...
        for (top, right, bottom, left), encoding in zip(locations, encodings):
            ...
            merged = False
            for b in blocks:
                if b.label == "person" and abs(b.x - face_cx) < self._face_merge_dist and abs(b.y - face_cy) < self._face_merge_dist:
                    b.id    = face_id
                    b.label = name
                    merged  = True
                    break
