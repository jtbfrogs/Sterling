...
class Sterling:
    ...
    def _init_memory(self):
        ...
        self._memory = Memory(
            ...
            facts_file=mem_cfg.get("facts_file", "facts.json"),
            recall_results=mem_cfg.get("recall_results", 3),
        )
    ...
    def _conversation_loop(self):
        ...
        try:
          while self._running:
            ...
            # Per-turn context notes. These are injected into THIS generation
            # only (via get_messages(ephemeral_context=...)) and are NEVER
            # stored in the conversation window. Keeping them out of memory is
            # what stops one turn's data ("now playing X") from bleeding into
            # the next turn ("tell me a joke").
            context_parts: list[str] = []

            # "Remember that ..." — durable facts the user wants kept
            fact = self._parse_remember_fact(text)
            if fact:
                self._memory.remember_fact(fact)
                self._tts.speak(self._config.get("sterling", {}).get(
                    "fact_saved_message", "Got it. I'll remember that."))
                logger.info(f"Fact stored: {fact}")
                if winding_down:
                    break
                continue
            ...
            if enroll_name and self._vision:
                ...
                context_parts.append(f"[Face enrollment result: {result}]")
                ...
                if full_response and not was_interrupted:
            ...
            if not intent_handled and self._vision and self._is_tracking_query(text):
                ...
                if result:
                    context_parts.append(f"[Object tracking: {result}]")
                else:
                    context_parts.append("[Object tracking: I haven't seen that item yet. It will be tracked next time it's visible on camera.]")
                intent_handled = True

            # Spotify control - execute immediately, let LLM respond naturally
            if not intent_handled and self._spotify:
                spotify_action = self._parse_spotify_intent(text)
                if spotify_action:
                    self._execute_spotify_command(spotify_action)
                    kind = spotify_action["action"]
                    spotify_context = None
                    if kind == "play":
                        q = spotify_action.get("query")
                        spotify_context = f"now playing '{q}' on Spotify" if q else "Spotify playback resumed"
                    elif kind == "pause":
                        spotify_context = "Spotify paused"
                    elif kind == "skip":
                        spotify_context = "skipped to next track on Spotify"
                    elif kind == "previous":
                        spotify_context = "went back to previous track on Spotify"
                    elif kind == "volume_up":
                        spotify_context = "Spotify volume turned up"
                    elif kind == "volume_down":
                        spotify_context = "Spotify volume turned down"
                    elif kind == "volume_set":
                        spotify_context = f"Spotify volume set to {spotify_action['value']}%"
                    elif kind == "now_playing":
                        result = spotify_action.get("result")
                        if result:
                            spotify_context = f"currently playing on Spotify: {result}"
                    if spotify_context:
                        context_parts.append(
                            f"[Action completed: {spotify_context}. Acknowledge in one "
                            f"short sentence — do not suggest alternatives or offer to do "
                            f"what was already done.]"
                        )
                    intent_handled = True
            ...
            if not intent_handled and self._govee and self._govee.has_devices:
                ...
                if light_action:
                    ...
                    context_parts.append("[Action completed: lights adjusted. Acknowledge briefly.]")
                    intent_handled = True
            ...
            if not intent_handled and self._workspace:
                ...
                if project_intent:
                    ...
                    if project_result:
                        context_parts.append(f"[{project_result}]")
                        intent_handled = True
            ...
            if self._is_weather_query(text):
                ...
                if location:
                    ...
                    if conditions:
                        context_parts.append(
                            f"[Use only the following data to answer — do not guess: {conditions}]"
                        )
            ...
            if not intent_handled and self._is_vision_query(text):
                if not self._vision:
                    context_parts.append("[Camera is offline — you cannot see anything.]")
                else:
                    scene = self._describe_scene(text)
                    context_parts.append(
            ...
            if self._is_time_query(text):
                ...
                context_parts.append(
                    f"[Current local time: {time_str}. Today is {date_str}.]"
                )
                logger.info(f"Time context injected: {time_str}, {date_str}")

            # Store the CLEAN user utterance; inject per-turn context ephemerally.
            self._memory.add_user(text)
            ...
            if self._llm_stream:
                full_response, was_interrupted = self._speak_interruptible(
                    self._tts.speak_streaming,
                    self._llm.stream(self._memory.get_messages(ephemeral_context=ephemeral)),
                )
            else:
                full_response = self._llm.chat(
                    self._memory.get_messages(ephemeral_context=ephemeral))
                _, was_interrupted = self._speak_interruptible(
                    self._tts.speak, full_response
                )

            if was_interrupted:
                self._memory.pop_last_user()  # don't let orphaned turn pollute next exchange
    ...
    def _handle_command(self, text: str) -> bool:
        ...
        # ── Forget stored facts ───────────────────────────────
        forget_phrases = cmds.get("forget_facts", [
            "forget what you know about me", "forget my facts",
            "delete what you know about me", "forget everything about me",
        ])
        if any(phrase in t for phrase in forget_phrases):
            n = self._memory.forget_facts()
            self._tts.speak(s_cfg.get(
                "forget_facts_message",
                "Done. I've cleared what I knew about you.",
            ))
            logger.info(f"Cleared {n} stored fact(s).")
            return True
    ...
    def _speak_interruptible(self, speak_func, *args) -> tuple[str, bool]:
        ...
        # Activate self-echo suppression: reject wake matches that are just
        # Sterling hearing his own voice through the speakers.
        self._wake_word.enable_echo_guard(lambda: self._tts.spoken_text)

        # Grace period before barge-in becomes active. The self-echo guard now
        # filters Sterling's own voice, so this can be short. Configurable via
        # conversation.interrupt_grace (seconds).
        grace = self._config.get("conversation", {}).get("interrupt_grace", 0.8)

        def _monitor():
            # Brief grace period: gives playback a moment to start and lets very
            # short replies finish cleanly. stop_event.wait() doubles as the
            # sleep and an early-exit: if TTS finishes during it, wait() returns
            # True and the monitor exits without ever listening.
            if stop_event.wait(timeout=grace):
                return  # TTS already finished/stopped during grace period
        ...
        try:
            ...
            self._wake_word.disable_echo_guard()
        ...
        if "play" in t:
            ...
            for f in ("me ", "some ", "a bit of "):
                if after.startswith(f):
                    after = after[len(f):]
            # Strip trailing filler so "Chance the Rapper for me?" → "Chance the Rapper"
            for f in (" for me please", " for me", " please", " right now", " now", " on spotify"):
                if after.endswith(f):
                    after = after[:-len(f)]
                    break
            after = after.rstrip("?.!")
            return {"action": "play", "query": after.strip() or None}
    ...
    def _parse_remember_fact(self, text: str) -> str | None:
        """
        Detect a request to remember a durable fact about the user.

        Triggers on:
            "remember that I like jazz"
            "remember I take my coffee black"
            "note that my sister's name is Anna"
            "don't forget I have a meeting at noon"
        Returns the fact text (rephrased to second person), or None.
        Excludes face-enrollment phrasing (handled separately).
        """
        t = text.strip()
        low = t.lower()

        # Skip face enrollment phrasings ("remember this person as ...")
        if re.search(r"remember (?:this|that) (?:person|face|guy|woman|man)", low):
            return None

        m = re.search(
            r"\b(?:remember|note|keep in mind|don'?t forget)\b(?:\s+that)?\s+(.+)",
            t, re.IGNORECASE,
        )
        if not m:
            return None
        ...
        if len(fact) < 3:
            return None
        # Light first->second person flip so stored facts read naturally
        fact = re.sub(r"^i\s+am\b", "the user is", fact, flags=re.IGNORECASE)
        fact = re.sub(r"^i\b", "the user", fact, flags=re.IGNORECASE)
        fact = re.sub(r"\bmy\b", "the user's", fact, flags=re.IGNORECASE)
        return fact

    def _describe_scene(self, text: str) -> str:
        """
        Produce a scene description for a vision query.

        Uses a vision-language model (moondream/llava via Ollama) when one is
        configured and available — this gives real pixel understanding for
        "what am I holding" / "describe the scene". Falls back to the fast
        YOLO + heuristic description otherwise.
        """
        vlm_model = self._config.get("vision", {}).get("vlm_model", "").strip()
        if vlm_model:
            try:
                jpeg = self._vision.get_frame_jpeg()
                if jpeg:
                    desc = self._llm.describe_image(
                        jpeg,
                        prompt=(
                            "You are the eyes of an assistant looking through a webcam. "
                            "In one or two short sentences, describe what is visible, "
                            "focusing on the person and what they are doing or holding. "
                            "Be specific and concrete."
                        ),
                        model=vlm_model,
                    )
                    if desc:
                        # Pair the VLM view with YOLO labels for grounding
                        labels = self._vision.get_scene_description()
                        return f"{desc.strip()} (detected objects: {labels})"
            except Exception as e:
                logger.warning(f"VLM scene description failed ({e}) — using YOLO fallback.")
        return self._vision.get_scene_description()
