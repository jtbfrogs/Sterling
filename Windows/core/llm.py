...
class LLM:
    ...
    def describe_image(self, image_bytes: bytes, prompt: str, model: str = None) -> str:
        """
        Send an image to a vision-language model (e.g. moondream, llava) and
        return a text description. Requires the VLM to be pulled in Ollama:
            ollama pull moondream

        Args:
            image_bytes: Raw JPEG/PNG bytes of the frame.
            prompt:      Instruction for what to describe.
            model:       VLM model name. Defaults to the main model (must be a VLM).

        Returns:
            Description text, or "" on failure.
        """
        try:
            response = self._client.chat(
            ...
            logger.error(f"VLM describe_image failed: {e}")
            return ""
