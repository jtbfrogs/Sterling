...
class WakeWordDetector:
    ...
    def __init__(
        ...
        # Self-echo guard: when active, transcriptions that mostly match what
        # Sterling is currently saying are rejected (he hears himself through
        # the speakers — no hardware echo cancellation).
        self._echo_text_fn = None   # callable() -> str (current TTS text)
    ...
    def enable_echo_guard(self, text_fn):
        """Activate self-echo suppression. text_fn returns Sterling's current speech."""
        self._echo_text_fn = text_fn

    def disable_echo_guard(self):
        self._echo_text_fn = None
    ...
    def _transcribe_and_check(self) -> bool:
        ...
        # Self-echo guard: if most of what we heard is contained in what Sterling
        # is currently saying, it's his own voice bleeding back through the mic.
        if self._echo_text_fn is not None and self._is_self_echo(text_norm):
            logger.debug(f"Ignored self-echo: {text!r}")
            return False
    ...
    def _is_self_echo(self, heard_norm: str) -> bool:
        """True if the heard transcript is largely a subset of Sterling's own speech."""
        try:
            spoken = self._normalise(self._echo_text_fn() or "")
        except Exception:
            return False
        if not spoken:
            return False
        ...
        if not heard_words:
            return False
        in_spoken = sum(1 for w in heard_words if w in spoken_words)
        # If half or more of the heard words appear in what Sterling just said,
        # treat it as an echo. A genuine wake word ("sterling" — which Sterling
        # never says) will not be in the spoken set, so real barge-in still works.
        return (in_spoken / len(heard_words)) >= 0.5
